...
- **ML-предсказания**: **Целевой KPI** после валидации на реальных исходах сигналов и OHLC — ориентир **≥87.2%** (не маркетинговый факт до бэктеста; см. `docs/ML_ACCURACY_DISCREPANCY.md`, `docs/ML_DATA_INTEGRITY_ROADMAP.md`).
...
3. **ML Service**:
   ...
   - Ensemble / XGBoost; **фактическая точность** задаётся после обучения на данных с реальными индикаторами и исходами
   - Текущий пайплайн `train_from_db.py`: порядка **8** основных признаков (расширение — по мере подключения рыночных фич)
...
**Критерии выполнения**:
- Функционирующий ML сервис; **целевая** точность на валидированном hold-out ≥87.2% (после наполнения `technical_indicators` / реальных исходов)
...
- Точность ML на **подтверждённой** выборке (out-of-sample, без синтетики) ≥87.2% — **целевой KPI**, не текущий статус продукта
- Покрытие тестами >80%
