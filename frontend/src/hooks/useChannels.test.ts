...
const mockChannels: Channel[] = [
  { id: '1', name: 'Test Channel 1', status: 'active', created_at: '2023-01-01', accuracy: 75, type: 'telegram' },
  { id: '2', name: 'Test Channel 2', status: 'inactive', created_at: '2023-02-01', accuracy: 60, type: 'twitter' },
];
...
describe('useChannels Hook', () => {
  ...
  it('exposes refetch and mutations', async () => {
    const { result } = renderHook(() => useChannels(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.channels.length).toBeGreaterThanOrEqual(0));
