...
export default function SubscriptionPage() {
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      const resp = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/stripe/create-checkout`, {
        method: 'POST',
                ...
                <div className="p-4 pt-0 mt-auto">
                  ...
                  >
                    {loading && selectedPlan === plan.id ? (
                      <><Loader2 className="h-4 w-4 animate-spin mr-2" />Переход к оплате...</>
                    ) : isCurrent ? 'Текущий план' : plan.price === 0 ? 'Начать бесплатно' : 'Оформить подписку'}
                  </Button>
                </div>
              </Card>
            );
