...
const AlertsPage: NextPage = () => {
  ...
  const resolved = signals.filter(s =>
    ((s.status as string) || '').includes('HIT')
  ).length;

  return (
    <>
      <Head>
        <title>Алерты - CryptoAnalytics</title>
      </Head>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <Alert variant="default" className="mb-6" role="status">
          <AlertDescription>
            ...
            <strong>AlertSystem</strong> (push по правилам, email/Telegram по
            подписке) в ТЗ запланирован как Pro-функция — пока не реализован;
            эта страница не является полноценным push-центром.
          </AlertDescription>
        </Alert>
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">
            Сигналы собранные из {channelCount} каналов
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {signals.length === 0 && (
                <p className="text-gray-500 text-center py-4">Нет сигналов</p>
              )}
              {signals.map((s, i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-medium">
                      ${Number(s.entry_price || 0).toLocaleString()}
                    </p>
