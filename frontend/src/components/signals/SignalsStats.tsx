...
export const SignalsStats: React.FC<SignalsStatsProps> = ({ signals }) => {
  ...
  const getStatCard = (title: string, value: string | number, subtitle?: string, color?: string) => (
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card>
        <CardBody className="p-4">
          {getStatCard('Активные', stats.active, 'в процессе', 'text-yellow-600')}
        </CardBody>
      </Card>
      ...
      <Card>
        <CardBody className="p-4">
          <div className="text-center">
            <div className="text-lg font-semibold text-gray-900 mb-2">Статусы</div>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <Badge variant="success" size="sm">Выполнено</Badge>
                <span className="text-sm">{stats.completed}</span>
              </div>
              <div className="flex justify-between items-center">
                <Badge variant="warning" size="sm">Активно</Badge>
                <span className="text-sm">{stats.active}</span>
              </div>
              <div className="flex justify-between items-center">
                <Badge variant="danger" size="sm">Не выполнено</Badge>
      ...
      <Card>
        <CardBody className="p-4">
          <div className="text-center">
            <div className="text-lg font-semibold text-gray-900 mb-2">Направления</div>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                <Badge variant="danger" size="sm">Short</Badge>
      ...
      {stats.bestSignal && (
        <Card>
          <CardBody className="p-4">
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-900 mb-2">Лучший сигнал</div>
              <div className="text-2xl font-bold text-green-600">
                +{stats.bestSignal.pnl!.toFixed(2)}%
              </div>
              <div className="text-sm text-gray-600">{stats.bestSignal.asset}</div>
      ...
      {stats.worstSignal && (
        <Card>
          <CardBody className="p-4">
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-900 mb-2">Худший сигнал</div>
              <div className="text-2xl font-bold text-red-600">
                {stats.worstSignal.pnl!.toFixed(2)}%
              </div>
              <div className="text-sm text-gray-600">{stats.worstSignal.asset}</div>
              <div className="text-xs text-gray-500 mt-1">
