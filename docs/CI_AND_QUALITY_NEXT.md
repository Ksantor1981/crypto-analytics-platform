...
- **Факт:** в `.github/workflows/ci.yml` для backend задано `--cov-fail-under=45` (цель ТЗ — выше).
- **План:** стабильно поднимать порог (например 45 → 50 → 60) только после того, как локально `pytest --cov=app` стабильно выше нового значения.
