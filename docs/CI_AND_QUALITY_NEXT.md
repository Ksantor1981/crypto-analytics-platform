...
- **Факт:** в `.github/workflows/ci.yml` для backend задано `--cov-fail-under=40` (цель ТЗ — выше).
- **План:** стабильно поднимать порог (например 40 → 45 → 60) только после того, как локально `pytest --cov=app` стабильно выше нового значения.
