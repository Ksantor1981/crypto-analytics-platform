...
| 3 | Shadow ingestion (`SHADOW_PIPELINE_ENABLED`), dual-write, метрики | 3–5 d | **In progress** — Telegram web + **Reddit** + **Telethon** (`raw_payload.mtproto`, `POST .../telethon-collect-all`, UI на `/channels`; Celery beat `collect_telethon_all_channels` при `CELERY_TELETHON_COLLECT_ENABLED`) |
