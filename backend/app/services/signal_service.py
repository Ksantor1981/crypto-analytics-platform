...
class SignalService:
    ...
    def get_signals_paginated(
        ...
        filters: Optional[SignalFilterParams] = None,
        user_subscription_tier: str = "FREE_USER"
    ) -> Tuple[List[Signal], Optional[int]]:
        """Get signals with cursor-based pagination."""
        query = self.db.query(Signal).options(joinedload(Signal.channel))

        # Apply filters (same as get_signals)
        if filters:
            if filters.channel_id:
                query = query.filter(Signal.channel_id == filters.channel_id)
        ...
        # For free users, limit to last 30 days
        if user_subscription_tier == "FREE_USER":
            thirty_days_ago = datetime.utcnow() - timedelta(days=30)
            query = query.filter(Signal.message_timestamp >= thirty_days_ago)
            limit = min(limit, 50)
        ...
        # Apply cursor
        if cursor:
            query = query.filter(Signal.id < cursor)  # Assuming ID is auto-incrementing

        # Get signals
        signals = query.order_by(desc(Signal.id)).limit(limit + 1).all()  # +1 to check if there are more

        # Determine next cursor
        next_cursor = None
        if len(signals) > limit:
            next_cursor = signals[-1].id
            signals = signals[:-1]  # Remove the extra one
