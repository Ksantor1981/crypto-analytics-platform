...
Проверка соответствия TASKS2.md - Этап 0.1.5 (ИСПРАВЛЕННАЯ ВЕРСИЯ)
"""
...
class FullCycleTest:
    ...
    def test_backend_auth(self) -> bool:
        ...
        try:
            ...
            if login_response.status_code == 200:
                ...
                self.log_test("Backend Authentication", "PASS", "Admin login successful")
                return True
            
            # Если админа нет, создаем нового пользователя
            register_data = {
                "email": f"test_{int(time.time())}@example.com",
                "password": "test123456",
                "confirm_password": "test123456",  # Добавлено обязательное поле
            ...
            if register_response.status_code == 201:
                ...
                if login_response.status_code == 200:
                    tokens = login_response.json()
                    self.access_token = tokens.get("access_token")
                    self.log_test("Backend Authentication", "PASS", "New user registered and logged in")
                    return True
                else:
                    self.log_test("Backend Authentication", "FAIL", f"Login after registration failed: {login_response.status_code}")
                    return False
            else:
                self.log_test("Backend Authentication", "FAIL", f"Registration failed: {register_response.status_code} - {register_response.text[:200]}")
                return False
    ...
    def test_backend_ml_integration(self) -> bool:
        ...
        try:
            ...
            ml_health_response = requests.get(f"{API_V1}/ml/health", timeout=10)
    ...
    def run_full_test_suite(self) -> Dict[str, Any]:
        ...
        with open("full_cycle_test_report_fixed.json", "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        
        print(f"💾 Отчет сохранен в: full_cycle_test_report_fixed.json")
        
        return report

if __name__ == "__main__":
    print("🔥 КРИТИЧЕСКИЙ ТЕСТ ПОЛНОГО ЦИКЛА (ИСПРАВЛЕННАЯ ВЕРСИЯ)")
